...
public class Wykres2 extends Application {

	// WYKRES PRZED TESTEM

	static String Lista[] = new String[Start.Odpowiedzi.length];

	public static void Start(Stage primaryStage, BorderPane root) {
		...
		for (int i = 0; i < Start.Odpowiedzi.length; i++) { //// Tworzenie Kolumn w wykresie w zale¿noœci od iloœci
															//// pytañ
			Lista[i] = "Pytanie " + (i + 1); /// Nazwa ka¿dej kolumny
			series1.getData().add(new XYChart.Data(Lista[i], Menu.p.get(i).getODPDOBRE())); //// Tworzenie Kolumn w
																							//// wykresie w zale¿noœci
																							//// od iloœci pytañ
			series2.getData().add(new XYChart.Data(Lista[i], Menu.p.get(i).getODPZ£E()));
		}
